...
namespace PhpSwitcher.Services
{
    public class PhpVersionService
    {
        ...
        public static bool IsVersionInstalled(AppConfig config, string version, bool isNTS)
        {
        ...
        public static async Task<List<PhpVersion>?> FetchAvailablePhpVersionsAsync()
        {
            ...
            try
            {
                ...
                LogService.Log($"Fetching content from: {url}", LogLevel.DEBUG);
                
                // Get the response
                string response = await HttpClient.GetStringAsync(url);
                
                LogService.Log($"Response received. Length: {response.Length} characters", LogLevel.DEBUG);
                ...
                LogService.Log($"Raw response saved to {responseFilePath} for inspection", LogLevel.WARNING);
                ...
                // Regular expression to match PHP version zip files for x64
                // Updated pattern to match both old format (VC\d+) and new format (vs\d+)
                string pattern = @"(php-(\d+\.\d+\.\d+)(?:-nts)?-Win32-(?:VC\d+|vs\d+)-x64\.zip)";
                
                LogService.Log($"Searching for pattern: {pattern}", LogLevel.DEBUG);
                
                var matches = Regex.Matches(response, pattern);
                
                LogService.Log($"Raw matches found: {matches.Count}", LogLevel.WARNING);
                
                // Verify that matches were found
                if (matches.Count == 0)
                {
                    // Try with an alternative pattern
                    string alternativePattern = @"href=""[^""]*/(php-(\d+\.\d+\.\d+)(?:-nts)?-Win32-(?:VC\d+|vs\d+)-x64\.zip)""";
                    LogService.Log($"No matches found with primary pattern. Trying alternative pattern: {alternativePattern}", LogLevel.WARNING);
                    matches = Regex.Matches(response, alternativePattern);
                ...
                foreach (Match match in matches)
                {
                    ...
                    // Create a unique key for this version+type combination to avoid duplicates
                    string uniqueKey = $"{version}-{isNts}";
                    
                    // Only add if we haven't seen this version+type combination before
                    if (!uniqueVersions.ContainsKey(uniqueKey))
                    {
                        string[] versionParts = version.Split('.');
                        string minorVersion = $"{versionParts[0]}.{versionParts[1]}";
                        int patchVersion = int.Parse(versionParts[2]);
                        ...
                        bool isLatestPatch = (patchVersion == minorVersions[minorVersion]);
                        ...
                        var versionInfo = new PhpVersion
                        {
                            ...
                            FileName = filename,
                            IsNTS = isNts,
                            DownloadUrl = $"https://windows.php.net/downloads/releases/archives/{filename}",
                        ...
                        LogService.Log($"Added version: {versionInfo.DisplayName} (last-patch: {isLatestPatch})", LogLevel.DEBUG);
                    }
                }
